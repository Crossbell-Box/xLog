...
export const loader: LoaderFunction = async ({ request, params }) => {
  ...
  const sites = await siteController.getSites(user)

  const subdomain = params.subdomain as string
  const site = sites.find((site) => site.subdomain === subdomain)
  if (!site) {
...
export default function DashboardLayout({}: {}) {
  ...
  const subdomain = params.subdomain as string
  const { sites, viewer } = useLoaderData()
  ...
  return (
    <>
      <div className="flex">
        <Sidebar>
          <div className="mb-2">
            <SiteSwitcher subdomain={subdomain} sites={sites} viewer={viewer} />
          </div>
